...
-- 1. ADD EMAIL DELIVERY TRACKING COLUMNS TO TEAMS TABLE
ALTER TABLE teams 
...
-- 2. ADD EMAIL DELIVERY TRACKING COLUMNS TO AUDIENCE REGISTRATIONS TABLE
...
-- 3. ENSURE PERFORMANCE & LOOKUP INDEXES
...
-- 4. DATABASE-LEVEL TEAM SIZE ENFORCEMENT (Strictly 2 Players per Squad)
...
-- Reads participant_limit dynamically from event_settings with ROW LOCK for race condition protection
...
DECLARE
  ...
  v_new_team_id UUID;
BEGIN
  -- 1. Check Event Settings with row lock (FOR UPDATE prevents simultaneous capacity bypass)
  SELECT registration_open, participant_limit INTO v_settings 
  FROM event_settings 
  LIMIT 1 
  FOR UPDATE;
  ...
  -- 2. Check Participant Limit dynamically from database settings
  ...
  -- 5. Insert Team
  INSERT INTO teams (team_id, name, game, qr_token, registration_status, check_in_status, email_notification_status)
  VALUES (p_team_id, trim(p_team_name), p_game, p_qr_token, 'confirmed', 'not_checked_in', 'pending')
...
-- Reads audience_limit dynamically from event_settings with ROW LOCK for race condition protection
...
BEGIN
  -- 1. Check Event Settings with row lock (FOR UPDATE prevents simultaneous capacity bypass)
  SELECT registration_open, audience_limit INTO v_settings 
  FROM event_settings 
  LIMIT 1 
  FOR UPDATE;
  ...
  -- 2. Check Audience Limit dynamically from database settings
  ...
  INSERT INTO audience_registrations (pass_id, full_name, email, phone, college_id, qr_token, registration_status, check_in_status, email_notification_status)
  VALUES (p_pass_id, trim(p_full_name), lower(trim(p_email)), trim(p_phone), trim(p_college_id), p_qr_token, 'confirmed', 'not_checked_in', 'pending')
...
-- 6. GRANT EXECUTION PERMISSIONS TO ROLES
